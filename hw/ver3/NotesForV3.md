...
## Circuit Updates
- [x] Split into two PCBs
  - BMS (Sophrosyne) logic control board
  - PWR (Ichnaea) power switching/balance/isolation board
- [x] ~~Increase the size of ESD protection diodes~~

### **System Sensor Improvements**
- [x] Input Current Sense (enable efficiency calculations)
- [x] 2x External temperature for heatsink temp (board temp isn't a sufficient indicator of power switch temp)
- [X] Voltage sample before inline load switch (enable matching to load before connection is made)

### **Cost Optimization**
- [x] ~~Smaller flash memory chip (2x 1MB?). Even the debug software really isn't that big and I don't need much space for logging/PDI.~~
  - The part is $0.37. Not much impact on optimization.
- [x] Cheaper buttons. We have so much board space they can be bigger.
- [x] USB uB Port instead of USB-C. Each USB-C is $1 and we're not even using USB-3.0 speeds.

### **Ichnaea**
- [x] ~~Dead-man switch to disable power output when Sophrosyne isn't connected~~
- [x] Add high current, high side load switch to enable safe connection to battery
- [x] ~~Add an active bleeder resistor to the output to act as a "short" for calibration and system safe-ing. 1k should be sufficient.~~
  - Inline power switch resolves this.
- [x] Improve control resolution on output voltage by varying Ra/Rb.
...
- [x] Try to get a single flat plane for component height. Ideally the power IC side of the system is covered in a giant block of aluminium.
- [x] Design mounts to allow for airflow. We have a bit of a heat problem.
- [x] 12v/5v capacitors on the bottom of the board were placed too close together. Give space to power capacitors.

#### **Cost Optimization**
- [x] Measure ripple currents under designed peak load and start removing capacitors (not cheap). How much do we really need?
  - Current BOM is 44x 100v 10uF MLCC @$0.31 ea: $13.64
  - ~~I think I could use electrolytics + ceramics and get the cost down.~~ (Tried this before. Caps blew up.)
- [x] Optimize MOSFET selection for cost/current/RdsON.
...
- [x] Are there cheaper gate drivers? Previously spent $1.83/10x == $18.30.
...
- [x] A 100k resistor across the SS pin causes the system to not boot properly. Removing fixes it, but could also go several MOhm(?).
  - Removed
- [x] A sufficiently cold temperature (5C) on the NTC thermistors can cause the sense output to exceed 3.3v, breaking the ADC input.
  - Swapped sense supply voltage from 5v to 3.3v
- [x] MCU doesn't boot from solar power application. Have to press reset button.
  - Suspect my addition of a 3.3v pullup + 100nF cap on the RUN pin caused this. Reference RP2040 schematic has none of these components.
